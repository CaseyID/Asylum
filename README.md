...
- Decisions are a first-class operator workflow: harness-awaited-input events auto-create pending decisions, Cockpit/CLI/MCP can list and resolve them (approve/deny/free-text answer), and the resolution is injected back into the node. ntfy replies correlated to a node with a pending decision resolve it the same way. Claude single-select menu questions carry their option labels on the decision, and an answer naming an option selects it in the menu itself (a non-matching answer errors and leaves the decision pending); multi-select and multi-question dialogs use the free-text path.
