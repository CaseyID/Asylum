...
- Phase B: in progress. Plan: plans/2026-07-06-phase-b-autonomy-loop.md. COMPLETE and merged to main: W1 (event ingestion + catalog truth + interrupt/exit fix, f7186bb), W0 (launch-prompt + send_input submit fixes, live-verified), W2 (harness-event CLI bridge, 329761c), W3 (launch injection of hooks/statusline/session-id/codex-notify, live-verified all four channels, 81df0cd). Remaining: W4 (hook actions + decision loop, in progress), W5 (cockpit), live gate. Delivered contracts per workstream are in the plan doc.
