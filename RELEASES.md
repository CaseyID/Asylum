...
- `harness-asylum-control`: on main, validated, not released. Adds real `node.spawn_peer` API/CLI/MCP capability plus local Codex/Claude Code Asylum MCP launch injection so command-center nodes can create peer nodes through the daemon. Subsumed by the completion-mission work below; release together.
- Completion mission 2026-07-06/07 (docs/superpowers/specs/2026-07-06-asylum-completion-mission.md): on main, NOT released — local-only by explicit owner instruction. Phases A-C merged and live-gated: harness-event ingestion + hook bridge (claude hooks/statusline, codex notify), input-delivery fixes (launch prompt + send_input submit), launch injection (pre-assigned session ids, hooks/statusLine settings, codex notify), send_input/spawn hook actions with recipe surface deleted, decision producer/feedback loop (incl. real ntfy reply correlation), cockpit surfacing (action forms, decisions, liveness truth, uptime/capacity/lineage/resume), Loon substrate rewritten against LoonV2 v0.1.5 (in-guest MCP over HTTP+token, live-verified in a real microVM), startup reconciliation + resume. Companion LoonV2 changes (installer artifact staging fix, destroyed-VM filter/prune) are merged on LoonV2 main and installed locally, also unreleased. When releasing, cut v0.1.11+ only after packaging validation and a codex re-auth spot check (see mission Open Questions).
