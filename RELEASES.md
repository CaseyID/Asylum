...
- update the ledger row + the active delivery plan's "Release status" section so the state is **legible**;
...
- **After a delivery cycle.** A planned multi-PR initiative is delivered → typically a release follows.
...
|---|---|---|---|---|---|---|
| 0.3.0 | v0.3.0 | 2026-07-13 | d13860f | **Published** | linux-x86_64, linux-arm64, darwin-arm64, darwin-x86_64 | Orchestration-alignment delivery: launch profiles end to end (`HARN-005`..`HARN-007`: per-node model/effort passed through verbatim, descriptor advertisement, honest unsupported errors, recorded profile surviving resume, Cockpit controls), menu-decision fidelity (`DECISION-004`: option capture via PreToolUse hook, answer-by-label arrow-key selection, honest error on unmatched answers), launch-packet layer-choice + verification etiquette (`LAYER-003`/`004`, drift-guarded), claude launch-prompt deliver-and-confirm on local and loon substrates (fixes claude 2.1.207 startup swallowing launch prompts), claude token telemetry from statusline context-window totals, 2.1.202 PTY crash closed not-reproducible. Every workstream adversarially reviewed pre-merge; live acceptance evidence in the plan doc's Delivery record (non-default model proven from process argv + served transcript, non-default menu selection proven end to end, loon port proven on a real microVM). All four platforms cross-built from Linux x86_64; signed checksums; packaging validated via `cargo test-asylum-release`. |
| 0.2.0 | v0.2.0 | 2026-07-07 | ee045f5 | **Published** | linux-x86_64, linux-arm64, darwin-arm64, darwin-x86_64 | Completion-mission delivery (PR #16): autonomy loop (harness-event ingestion via claude hooks/statusline + codex notify, launch injection with pre-assigned session ids, send_input/spawn hook actions, decision producer/feedback with ntfy reply correlation), Loon substrate rewritten against LoonV2 (in-guest MCP over HTTP+token; companion LoonV2 v0.1.6 released same day), startup reconciliation + resume with graceful /exit stop, cockpit surfacing (action forms, decisions, liveness truth, uptime/capacity/lineage/resume), recipes surface removed for launch packets, full adversarial-review resolution. All four platforms cross-built from Linux x86_64; signed checksums. Packaging validated via `cargo test-asylum-release`. Codex re-auth spot check deferred (host codex OAuth needs interactive login; owner deprioritized) — see mission Open Questions. |
| 0.1.10 | v0.1.10 | 2026-05-07 | dbd5b18 | **Published** | linux-x86_64, linux-arm64, darwin-arm64, darwin-x86_64 | Validation-driven Cockpit fix delivery: real xterm.js terminal in NodeSession, signed browser session page, daemon PATH resolution + login-shell fallback, harness workspace pre-trust + bypass flags, launch-packet positional-prompt delivery, harness-exit liveness detection, optimistic send-input echo, `asylum doctor` PATH-mismatch diagnosis, plus cockpit polish (Notifications rename, Hooks empty-states, first-run modals, post-launch navigation, accurate "binary missing" copy). |
| 0.1.9 | v0.1.9 | 2026-05-07 | a840fa7 | **Published** | linux-x86_64, linux-arm64, darwin-arm64, darwin-x86_64 | Cargo source workflow delivery: repo-local xtask-backed run/build/test flows, Vite API/WebSocket proxying, source dev isolation under `.asylum-dev` on port 7788, and Cargo target-dir aware daemon launch. |
| 0.1.8 | v0.1.8 | 2026-05-06 | cc34809 | **Published** | linux-x86_64, linux-arm64, darwin-arm64, darwin-x86_64 | Hotfix: stop Cockpit replaying historical signed-session events as new browser actions, preventing popup loops; adjust session UI labels. |
...
| 0.1.5 | v0.1.5 | 2026-05-05 | 0403516 | **Published** | linux-x86_64, linux-arm64, darwin-arm64, darwin-x86_64 | Architecture refactor: four-crate shape (`asylum`, `asylum-cli`, `asylum-daemon`, `asylum-types`), `asylum daemon run` foreground daemon mode, Unix-socket CLI/MCP local control, Cockpit remains HTTP/WebSocket. |
...
| 0.1.3 | v0.1.3 | 2026-04-30 | 0b8a7a4 | Published | linux-x86_64, linux-arm64, darwin-arm64, darwin-x86_64 | CLI composability + uninstall delivery: HostState introspection layer, `asylum status --json`, `asylum uninstall`, `asylum service generate` (replaces `asylum install systemd\|launchd`), thin `install.sh` (857->593 lines). |
...
None as of v0.3.0 (2026-07-13).

Resolved history:

- Codex re-auth spot check (carried from v0.2.0): RESOLVED 2026-07-09 — owner ran `codex login` and a one-node live re-check passed on the installed v0.2.0 daemon (codex node replied, notify-bridge harness event posted, clean stop).
- (2026-07-07) `harness-asylum-control` and the completion mission both shipped in v0.2.0. Companion LoonV2 fixes shipped as LoonV2 v0.1.6 (GitHub release with manifest + role-based installer; host+client reinstalled locally from the published release and smoke-checked with a real microVM run).
...
- Active delivery plans in `docs/superpowers/plans/` — each delivery's "Release status" section links here
