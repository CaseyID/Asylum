...
- Codex re-auth spot check (carried from v0.2.0): host codex OAuth needs an interactive `codex login`, then a one-node live re-check of codex launches on the released binary. Deferred out of the v0.2.0 cut by owner decision (claude-first); v0.2.0 shipped with packaging validation only.
