...
|---|---|---|---|---|---|---|
| 0.1.10 | v0.1.10 | 2026-05-07 | _pending_ | _Pending_ | _pending build_ | Validation-driven fix delivery from the [2026-05-07 cockpit basic-flow validation report](docs/reviews/asylum-validation-report.md): real xterm.js terminal in NodeSession, browser-attach HTML page, daemon PATH resolution + login-shell fallback, harness workspace pre-trust + bypass flags, launch-packet positional-prompt delivery, harness-exit liveness detection, optimistic send-input echo, `asylum doctor` PATH-mismatch diagnosis, plus cockpit polish (Notifications rename, Hooks empty-states, first-run modals, post-launch navigation, accurate "binary missing" copy). |
