...
| 0.1.9 | v0.1.9 | 2026-05-07 | a840fa7 | **Published** | linux-x86_64, linux-arm64, darwin-arm64, darwin-x86_64 | Cargo dev workflow delivery: repo-local `cargo dev`/`build-stack`/`test-stack`/`run-stack` via xtask, Vite API/WebSocket proxying, source dev isolation under `.asylum-dev` on port 7788, Cargo target-dir aware daemon launch, and installed lifecycle aliases that delegate to the installed `asylum` binary. |
